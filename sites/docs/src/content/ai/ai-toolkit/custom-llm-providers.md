...
* The [Firebase AI Logic provider][],
  ...
  [Firebase AI Logic provider][]，封装 `firebase_ai` package
...
1. Configuration
   ...
   配置要在自定义提供商中支持完整可配置性，
   应让用户创建底层模型并作为参数传入，如 `MyLlmProvider`：
...
历史变更时，底层聊天需自动保持同步
（如 Firebase provider 在调用聊天专用方法时），或手动重建
（如 Firebase provider 在手动设置历史时）。
...
映射到底层 LLM 所处理的类型。
例如，Firebase provider 将 AI 工具包的 `ChatMessage` 映射为
...
AI 工具包中的 Firebase provider 处理配置与历史，
...
[Firebase provider][] 实现是构建自定义提供商的良好起点。
