...
在一般的开发过程中，我们可以使用 `flutter run` 命令，或者 IntelliJ 工具栏中的 **Run** 和 **Debug** 来测试 app。这时候，Flutter 默认会为我们构建 app 的 **debug** 版本。
...
当想要发布 app 时，比如[发布到 Google Play Store][play]，可以按照以下步骤来为 Android 准备 **发布** 版本。
...
* `application`：编辑 [`application`][applicationtag] 标签中的 `android:label` 来表示 app 的最终名字。
...
* `defaultConfig`：
  ...
  * `minSdkVersion` & `targetSdkVersion`：指定支持的最低的 API 版本，以及我们的 app 的目标 API 版本。具体可以参考[版本文档][versions]中的 API 版本部分。
...
当创建一个新的 Flutter app 的时候，它会有一个默认的启动图标。要自定义这个图标，可以参考 [Flutter Launcher Icons]({{site.pub}}/packages/flutter_launcher_icons)。
...
1. Review the [Android Launcher Icons][launchericons] guidelines for icon
   design.
   检查 [Android Launcher Icons][launchericons] 指南中图标设计部分。
...
**注意：** `keytool` 可能不在我们的系统路径中。它是Java JDK 的一部分，会在安装 Android Studio 的时候被一起安装。运行 `flutter doctor -v`，'Java binary at:' 之后打印出来的就是它的路径，然后用 `java` 来替换以上命令中的 `keytool`，并加上 `keytool` 的完整路径即可。
...
```
   buildTypes {
       release {
           // TODO：为发布版本添加自己的签名配置
           // 暂时采用调试密钥来签名，让 `flutter run --release` 可以先工作起来
           signingConfig signingConfigs.debug
       }
   }
```
...
reverse engineering.
默认情况下，Flutter 不会做混淆或者缩小 Android host 的工作。如果 app 使用了第三方的 Java 或者 Android 库，我们会希望 APK 变得更小，或者保护代码不被反编译出来。
...
关于混淆 Dart 代码的信息，可以参考 [Flutter wiki]({{site.github}}/flutter/flutter/wiki) 上的 [Obfuscating Dart Code]({{site.github}}/flutter/flutter/wiki/Obfuscating-Dart-Code)。
...
rules to be added.
这个配置只是保护 Flutter 引擎库。如果想要保护其他的库（例如，Firebase），需要为他们添加自己的规则。
...
1. Run `flutter build apk` (`flutter build` defaults to `--release`).
   运行 `flutter build apk` （`flutter build` 默认带有 `--release`）。
...
发布一个 app 到 Google Play Store 的详细说明，可以参考 [Google Play publishing documentation][play]。
...
这个部分描述了如何构建一个发布的 app bundle。如果在前面的部分已经完成了签名步骤，发布的bundle 会被签名。
...
1. [部署 APK ](https://developer.android.com/studio/command-line/bundletool#deploy_with_bundletool)到设备上
...
to the Play Store.
   按照 [这些步骤上传你的 bundle](https://developer.android.com/studio/publish/upload-bundle)
 到 Play Store上。
