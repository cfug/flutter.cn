...
你的开发环境必须满足 [Flutter 对 macOS 系统的版本要求][] 并 [已经安装 Xcode][]。 Flutter 支持 iOS 8.0 及以上。
...
为了将 Flutter 集成到你的既有应用，第一步要创建一个 Flutter module。
...
Flutter module 会创建在 `some/path/my_flutter/` 目录。在这个目录中，你可以像在其它 Flutter 项目中一样，执行 `flutter` 命令。比如 `flutter run --debug` 或者 `flutter build ios`。你也同样可以在 [Android Studio/IntelliJ][] 或者 [VS Code][] 中运行这个模块，并附带 Flutter 和 Dart 插件。
在集成到既有应用前，这个项目在 Flutter module 中包含了一个单视图的示例代码，对 Flutter 侧代码的测试会有帮助。
...
{{site.alert.note}}
iOS 代码要添加到你的既有应用或者 Flutter plugin中，而不是 Flutter module 的 `.ios/` 目录下。 `.ios/` 下的改变不会集成到你的既有应用。
在 `my_flutter` 执行 `flutter clean` 或者 `flutter pub get` 会重新生成这个目录。
{{site.alert.end}}
...
{{site.alert.note}}
你的应用将不能在模拟器上运行 Release 模式，因为 Flutter 还不支持将 Dart 代码编译成 x86 ahead-of-time (AOT) 模式的二进制文件。你可以在模拟机和真机上运行 Debug 模式，在真机上运行 Release 模式。
{{site.alert.end}}
...
你应用的 Debug 和 Release 编译配置，将会集成相对应的 Debug 或 Release [Flutter 编译模式][]。可以增加一个 Profile 编译配置用于在 profile 模式下测试应用。
...
除了上面的方法，你也可以创建必备的 frameworks，手动修改既有 Xcode项目，将他们集成进去。当你组内其它小伙伴不能在本地安装 Flutter SDK 和 CocoaPods，或者你不想使用 CocoaPods 作为既有应用的依赖管理时，这种方法会比较合适。但是每当你在 Flutter module 中改变了代码，都必须运行 `flutter build ios-framework`。
...
如果你使用前面的 [使用 CocoaPods 和 Flutter SDK 集成](#选项-a---使用-cocoapods-和-flutter-sdk-集成) ，你可以跳过本步骤。
...
在 Xcode 中将生成的 frameworks 集成到你的既有应用中。例如，你可以在 `some/path/MyApp/Flutter/Release/` 目录拖拽 frameworks 到 你的应用 target 编译设置的 General > Frameworks, Libraries, and Embedded Content 下。然后在 Embed 下拉列表中选择 "Embed & Sign"
...
{{site.alert.tip}}
如果你想在 Debug 编译配置下使用 Debug 版本的 Flutter frameworks，在 Release 编译配置下使用 Release 版本的 Flutter frameworks，在 `MyApp.xcodeproj/project.pbxproj` 文件中，尝试在所有 Flutter 相关 frameworks 上使用 `path = "Flutter/$(CONFIGURATION)/example.framework";` 替换 `path = Flutter/Release/example.framework;` 。（注意添加`"`）
