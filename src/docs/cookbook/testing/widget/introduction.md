...
为了实现这个目的，我们使用 `flutter_test` 包提供的顶级 [`find`]({{api}}/flutter_test/find-constant.html) 方法来创建我们的 `Finders`。因为我们要查找的是 `Text` widgets，所以可以使用 [`find.text`]({{api}}/flutter_test/CommonFinders-class.html) 方法。
