...
可以放在 `CustomScrollView` 里的可滚动列表和组件我们称之为 slivers。有几种类型的slivers，比如 `SliverList`，`SliverGridList` 和 `SliverAppBar`！实际上，`ListView` 和 `GridView` 组件底层使用的就是 `SliverList` 和 `SliverGrid`！
