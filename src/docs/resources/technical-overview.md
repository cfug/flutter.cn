...
开始开发之前无需移动开发经验。应用使用 [Dart](https://www.dartlang.org) 开发，如果你曾使用过 Java 或者 JavaScript 语言，那么 Dart 看上去会很熟悉。面向对象编程的经验对开发毫无疑问会有帮助，但即便不是程序员也可以制作 Flutter 应用！
...
widget 通过组合形成一个层次结构。每个 widget 嵌套其中，从其父亲节点继承属性。没有单独的“应用程序”对象，相反，根 widget 承担这一角色。
...
widget 本身往往是由许多小的、具有单一用途的 widget 组成，它们组合在一起可以产生强大的效果。比如，[Container](https://github.com/flutter/flutter/blob/master/packages/flutter/lib/src/widgets/container.dart) 是一个常用的 widget，由负责布局、绘制、定位和尺寸的几个 widget 组成。具体来说，Container 是由 [LimitedBox](https://docs.flutter.io/flutter/widgets/LimitedBox-class.html),
[ConstrainedBox](https://docs.flutter.io/flutter/widgets/ConstrainedBox-class.html),
[Align](https://docs.flutter.io/flutter/widgets/Align-class.html),
[Padding](https://docs.flutter.io/flutter/widgets/Padding-class.html),
[DecoratedBox](https://docs.flutter.io/flutter/widgets/DecoratedBox-class.html),
和 [Transform](https://docs.flutter.io/flutter/widgets/Transform-class.html) widget 组成的。你可以将这些与其它小而简单的 widget 用新颖的方式组合，而不是子类化 Container 以生成自定义效果。
...
你也可以通过组合其它 widget 以达到控制一个 widget 的*布局*。例如，为了居中一个 widget，你可以把它包裹在一个 Center widget 中。还有用于设置 间距、对齐、行、列以及网格的 widget，这些布局 widget 本身并没有视觉表现。反之，他们唯一的目的是在某些方面上控制另一个 widget 的布局。要了解一个 widget 以某种方式呈现的原因，查看其邻近 widget 通常很有帮助。
...
这样做的目的是为了帮助你用更少的代码完成更多工作。例如，通过在 widget 层组合基础 widget 来构建 Material 层，而 widget 层本身则是通过对来自 Rendering 层的低层次对象组合而来。
...
这些层次为构建应用程序提供了许多选项。选择一种自定义方法来释放框架的全部表现力，或使用 widget 层中的构建块，或混合搭配。你可以组合 Flutter 提供的现成 widget，或使用 Flutter 团队在构建框架时所使用的相同工具和技术来构建你的自定义 widget。
...
没有什么是对你隐藏的。 你可以在生产力上从高级别、统一的 widget 概念中获益，且不会牺牲你想深入更底层时的能力。
...
一个 widget 的构建函数应该没有副作用。不论何时调用，widget 总应该返回一颗新的 widget 树，不论其之前返回了什么。框架会将先前构建与当前构建仔细进行比较，并确定需要对用户界面做哪些修改。
...
将状态与 widget 对象分开可以让其他 widget 以相同的方式处理无状态和有状态的 widget，而不用担心丢失状态。不同于通过维持一个子节点来保留状态，父节点可以自由地创建子节点实例，而不会丢失子节点的持久状态。框架会在适当的时候做完查找和重用现有状态对象的所有工作。
...
跟踪 Flutter 项目并通过多种方式参与讨论。我们开源并热衷听到来自你的声音。
