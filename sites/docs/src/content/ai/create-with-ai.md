...
* [Firebase AI Logic][] - The official Firebase SDK for using generative AI
  ...
  兼容 Gemini Developer API 或 Vertex AI。
  入门请参阅 [官方文档][firebase-ai-logic-docs]。
...
*   **Type-safe schemas**: Define strongly-typed inputs and outputs for AI
    ...
    **类型安全 schema**：使用 [`schemantic`](https://pub.dev/packages/schemantic) package 
    为 AI 交互定义强类型输入与输出。
...
是适用于 Visual Studio Code、JetBrains IDE、Android Studio 等 IDE 的 AI 协作工具。
它深度理解项目代码库，可帮助你：
...
Dart 与 Flutter MCP server 促成这一通信。
...
入门请参阅
[Dart 与 Flutter MCP server][dart-mcp-dart-docs] 官方文档。
