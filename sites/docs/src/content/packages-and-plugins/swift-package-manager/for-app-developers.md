...
请 [提交 issue][open an issue]。
Swift Package Manager 支持 [默认关闭][off by default]。
...
1. **Provides access to the Swift package ecosystem**.
   ...
   **可访问 Swift package 生态**。
   Flutter 插件可使用不断增长的 [Swift packages][] 生态。
...
1. **Simplifies Flutter installation**.
   ...
   **简化 Flutter 安装**。
   Xcode 已包含 Swift Package Manager。
   若项目使用 Swift Package Manager，则无需安装 Ruby 和 CocoaPods。
