...
Flutter 组件是由一个现代框架构建的，它的灵感来源于 [React](https://reactjs.org)。 它的核心思想是用组件来构建你的 UI 界面。组件描述了在当前的配置和状态下，视图所应该呈现的样子。当组件的状态改变，框架会通过比较渲染的树结构，来获得最小的改变，并更新状态。
...
**注意：** 如果你想熟练掌握 Flutter ，请查阅[在 Flutter 中构建布局](/docs/development/ui/layout)和[如何在 Flutter 工程中进行交互](/docs/development/ui/interactive)这两篇文章。
...
一个最基小的 Flutter 工程只需要在组件中调用 [runApp()][] 方法。
...
[runApp()][] 会持有组件 [Widget]({{api}}/widgets/Widget-class.html)，并且使该组件成为组件树结构中的根节点。在这个实例中：一棵组件树有两个组件，
[Center]({{api}}/widgets/Center-class.html) 组件及其子组件，[Text]({{api}}/widgets/Text-class.html) 组件。框架在布局时，根组件会铺满整个屏幕，也就是说 ”Hello World”
...
在写应用的过程中，你可能会创建一个新的组件继承[无状态组件]({{api}}/widgets/StatelessWidget-class.html)或[有状态组件]({{api}}/widgets/StatefulWidget-class.html)，
这取决于该组件是否有管理组件的状态。
组件的主要工作是是实现 [build]({{api}}/widgets/StatelessWidget/build.html) 方法。
该方法描述了其他更低层级的组件。框架会逐一构建这些组件直到最底层的计算和描述组件的[渲染对象]({{api}}/rendering/RenderObject-class.html)组件构建完成。
 ...
 * [Text]({{api}}/widgets/Text-class.html):
   [Text]({{api}}/widgets/Text-class.html) 组件是用来在应用内创建带样式的文本。
 ...
 * [Row]({{api}}/widgets/Row-class.html)，
   [Column]({{api}}/widgets/Column-class.html): 这些布局组件，可以在水平([Row]({{api}}/widgets/Row-class.html))和垂直([Column]({{api}}/widgets/Column-class.html))方向灵活布局，它们是基于Web的盒式布局模型而设计的。
...
* [Stack]({{api}}/widgets/Stack-class.html)：和以线性(水平或垂直)布局为主的组件不一样，
   [Stack]({{api}}/widgets/Stack-class.html) 组件可以让组件居于绘制顺序的顶部。
   你可以让[指定位置]({{api}}/widgets/Positioned-class.html)组件成为 
   [Stack]({{api}}/widgets/Stack-class.html) 的子组件，居于栈的上，右，下，左的位置。栈是基于 Web 中的绝对位置布局模型设计的。
 ...
 * [Container]({{api}}/widgets/Container-class.html): [Container]({{api}}/widgets/Container-class.html) 组件可以用来创建一个可见的矩形元素。
容器可以使用 [BoxDecoration]({{api}}/painting/BoxDecoration-class.html)
...
为了继承主题，很多组件需要在 [MaterialAPP]({{api}}/material/MaterialApp-class.html)
 中合理展现，因此可以以 [MaterialAPP]({{api}}/material/MaterialApp-class.html) 的方式运行应用程序。
...
上例中的 MyAppBar 组件创建了一个高为56像素的 [Container]({{api}}/widgets/Container-class.html)，
 左右内边缘为8像素。在容器里面，MyAppBar 以 [Row]({{api}}/widgets/Row-class.html) 
布局来布局子元素。中间子元素也就是标题组件，设为 [Expanded]({{api}}/widgets/Expanded-class.html) 
填充，也就是它要填充剩余空间。你可以定义多个[Expanded]({{api}}/widgets/Expanded-class.html) 
子元素，然后用 [flex]({{api}}/widgets/Expanded-class.html#flex) 属性设置每个子元素所占的比进行 [Expanded]({{api}}/widgets/Expanded-class.html)。
...
MyScaffold 以垂直方式来进行布局。在顶部放了一个 MyAppBar 组件实例，并把 [Text]({{api}}/widgets/Text-class.html) 组件传给它来作为应用的标题。把组件作为参数传递给其他的组价是一个很强大的技术，它可以让你以各种方式创建一些
可重用的通用组件。最后，MyScaffold 使用可 [Expanded]({{api}}/widgets/Expanded-class.html)
 的主体部分填充剩余的空间，主体部分分包含了一些重要的信息。

More information: [Layouts](/docs/development/ui/widgets/layout)

更多详细信息可以参考：[Layouts](/docs/development/ui/widgets/layout)布局
...
Flutter 提供了很多组件来构建一个应用，这些组件都遵守 Material Design 原则。一个 Material 应用使用 [MaterialApp]({{api}}/material/MaterialApp-class.html) 组件，
它可以在应用的根组件上创建很多有用的组件。包括[导航栏]({{api}}/widgets/Navigator-class.html)，
它管理一系列标为字符串的组件，也就是路由。[导航栏]({{api}}/widgets/Navigator-class.html)
可以在应用的页面中平滑的切换。使用 [MaterialApp]({{api}}/material/MaterialApp-class.html) 是可选的但是是一个很好的实践。
...
既然我们已经从 myAppBar 和 MyScaffold 说到 material.dart 中的 [AppBar]({{api}}/material/AppBar-class.html)
 和 [Scaffold]({{api}}/material/Scaffold-class.html) 组件，我们可以看下应用程序更多属性。
...
注意到我们又把组件作为参数传给别的组件。
 [Scoffold]({{api}}/material/Scaffold-class.html) 用一系列的组件作为参数，
每个组件都布局在 Scofford 的合适位置。同样的, [AppBar]({{api}}/material/AppBar-class.html#leading)
 组件也允许我们传递组件给[标题]({{api}}/material/AppBar-class.html#title)
组件作为组件的展示[样式]({{api}}/material/AppBar-class.html#leading)和
[行为]({{api}}/material/AppBar-class.html#actions)。这种方式是在这个框架中出现的，也是我们在设计自定义组件时所需要考虑的一点。

More information: [Material components](/docs/development/ui/widgets/material)

更多信息可以参考：[Material  components](/docs/development/ui/widgets/material)
...
注意：Material 是 Flutter 中两个自带的设计之一，如果想参考以 iOS 为主的设计，可以参考 [Cupertino component](/docs/development/ui/widgets/cupertino)，
它包含 [CupertinoApp]({{api}}/cupertino/CupertinoApp-class.html),
[CupertinoNavigationBar]({{api}}/cupertino/CupertinoNavigationBar-class.html) 等。
</aside>
...
[手势检测]({{api}}/widgets/GestureDetector-class.html)组件在界面上是看不到的，但是它能识别用户的手势。
当用户触碰[容器]({{api}}/widgets/Container-class.html)，[手势检测]({{api}}/widgets/GestureDetector-class.html)
会调用[onTap]({{api}}/widgets/GestureDetector-class.html#onTap)回调，此时会在控制台上输出一行信息。
你可以使用[手势检测]({{api}}/widgets/GestureDetector-class.html)组件检测各种输入手势，包括点击，拖拽，缩放。
...
很多组件使用[手势检测组件]({{api}}/widgets/GestureDetector-class.html)来为其他组提供可选的回调，例如
 [IconButton]({{api}}/material/IconButton-class.html)，当用户点击组件时，[RaisedButton]({{api}}/material/RaisedButton-class.html)，
[FloatingActionButton]({{api}}/material/FloatingActionButton-class.html) 等都有 [onPressed]({{api}}/material/RaisedButton-class.html#onPressed)
 回调。


More information: [Gestures in Flutter](/docs/development/ui/advanced/gestures)

更多信息请参考：[Flutter 中的手势识别](/docs/development/ui/advanced/gestures)
...
到目前为止，我们只使用了无状态组件。无状态组件接收的参数来自于它的父组件，并将参数存为 [final]({{site.dart-site}}/guides/language/language-tour#final-and-const)
 成员变量。当需要[创建]({{api}}/widgets/StatelessWidget/build.html)组件时，就是用这些存储的变量为创建的组件生成新的参数。
...
参考使用上文提到过的 [RaisedButton]({{api}}/material/RaisedButton-class.html) 来实现的一个简单示例：
...
组件是临时对象，用于展现当前状态下应用的展现。另一方面，状态对象在调用和[创建]({{api}}/widgets/State/build.html)
之间是持久的，以此来存储信息。
...
更多信息可参考:
- [有状态组件]({{api}}/widgets/StatefulWidget-class.html)
- [状态设置]({{api}}/widgets/State/setState.html)
...
primary color from the current theme, and another that uses gray.
 ...
 `ShoppingListItem` 类继承自无状态组件,它接收并存储构造函数的 [final]({{site.dart-site}}/guides/language/language-tour#final-and-const)
成员变量，这些值在 [build]({{api}}/widgets/StatelessWidget/build.html) 
...
当用户点击列表中的一项，组件不会直接改变 `inCart` 
 的值，而是通过父组件调用 `onCartChanged` 函数。这种方式可以在组件的生命周期中存储状态更长久，从而使状态持久化。理想情况下，组件的传给 [runApp()][] 的状态可以持久到整个应用的生命周期。
...
因为重新[渲染对象]({{api}}/rendering/RenderObject-class.html)
...
示例中 `ShoppingList` 继承自[有状态组件]({{api}}/widgets/StatefulWidget-class.html)，就是说这个组件会存储状态的变化，当 <font color=#0099ff>ShoppingList</font> 首次插入到渲染树结构中，框架会调用 
 [createState]({{api}}/widgets/StatefulWidget-class.html#createState) 方法创建一个
书中对应位置的 `_ShoppingListState` 实例(注意到 [State]({{api}}/widgets/State-class.html) 的子类通常以下划线开头来命名，表示它们的实现细节是私有的)。当该组件的父组件重新创建时，
父组件首先创建一个 `ShoppingList` 的实例，但是框架会复用之前创建的 `_ShoppingListState` ，而不会重新调用 
...
为了访问当前 `ShoppingList` 的属性， `_ShoppingListState` 可以使用它的
[组件]({{api}}/widgets/State-class.html#widget)属性，当父组件重新创建一个新的 `ShoppingList`，`_ShoppingListState` 会使用属性值来创建。如果你想在
[组件]({{api}}/widgets/State-class.html#widget)属性变化时接收到通知，你可以继承 
[didUpdateWidget]({{api}}/widgets/State-class.html#didUpdateWidget) 方法，
该方法会传递给老的组件，这样你就可以比较 `老组件` 和当前[组件]({{api}}/widgets/State-class.html#widget)。
...
当处理  `onCartChanged`  回调时， `_ShoppingListState` 通过增加或删除 `_shoppingCart` 购物车中的产品来计算最终的状态。
在调用 [setState]({{api}}/widgets/State-class.html#setState) 时，会通知框架改变最终的状态。调用 
[setState]({{api}}/widgets/State-class.html#setState) 标志着这个组件时脏的，
在屏幕下次更新时，组件也需要重新创建。当你改变了组件的最终状态而忘了调用 [setState]({{api}}/widgets/State-class.html#setState)，
框架不知道组件是脏组件，也就不会新 [创建]({{api}}/widgets/StatelessWidget/build.html)
组件，用户接口就不会对状态的改变做更新。
...
在有状态组件调用 [createState]({{api}}/widgets/StatefulWidget-class.html#createState)
 后，框架插入一个新的状态对象到树中，然后在状态对象中调用 
 [initState]({{api}}/widgets/State-class.html#initState)。
 [State]({{api}}/widgets/State-class.html) 的子类可以实现
 [initState]({{api}}/widgets/State-class.html#initState) 做一些只会发生一次的初始化工作。
例如配置动画或订阅平台服务。实现 [initState]({{api}}/widgets/State-class.html#initState) 
需要调用父类的 [super.initState]({{api}}/widgets/State-class.html#initState)方法。
...
当不在需要状态对象，框架会对该对象调用 [dispose]({{api}}/widgets/State-class.html#dispose)
 方法。你可以继承来清理状态。例如，你可以实现 [dispose]({{api}}/widgets/State-class.html#dispose)
 来取消计时器或者取消订阅平台服务。实现 [dispose]({{api}}/widgets/State-class.html#dispose)
 也需要调用父类的 [super.dispose]({{api}}/widgets/State-class.html#dispose) 方法。

More information: [State]({{api}}/widgets/State-class.html)

更多信息请参考: [State]({{api}}/widgets/State-class.html)
...
[运行时类型]({{api}}/widgets/Widget-class.html#runtimeType)来匹配当前的组件。
使用 [Keys]({{api}}/widgets/Widget-class.html#key) 时，
框架要求组件在的类型保持和[运行时类型]({{api}}/widgets/Widget-class.html#runtimeType)一致。
...
当同一组件需要构建多个实例时，Keys 就很有用。例如， `ShoppingList` 组件，就需要构建多个 
`ShoppingListItem` 实例来填充可见区域。
...
更多信息参考：[Key API]({{api}}/foundation/Key-class.html)
...
更多信息参考：[GlobalKey API]({{api}}/widgets/GlobalKey-class.html)
