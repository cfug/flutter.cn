...
要热重载 Flutter 应用程序：
 ...
 1. Run the app from a supported [Flutter editor](/docs/get-started/editor)
    ...
    在支持 [Flutter 编辑器](/docs/get-started/editor) 或终端窗口运行应用程序。物理机或虚拟器都可以。
...
作为一般规则，如果修改后的代码位于根 widget 的构建方法的下游，则热重载将按预期运行。
但是，如果修改后的代码不会因重新构建 widget 树而重新执行的话，那么在热重载后您将看不到它的效果。
