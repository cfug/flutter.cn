---
title: Cookbook
title: 实用教程
description: The Flutter cookbook provides recipes for many commonly performed tasks.
description: Flutter cookbook 为许多常见使用应用场景提供了指南。
tags: cookbook, 实用教程
keywords: Flutter教程,Flutter入门,Flutter实战,Flutter进阶
---

This cookbook contains recipes that demonstrate how to solve common problems 
while writing Flutter apps. Each recipe is self-contained and can be used as a
reference to help you build up an application.

本节内容包含了一个又一个的实用教程，帮助你解决编写 Flutter 应用中的常见问题。

## Animation
- [为页面切换加入动画效果](/docs/cookbook/animation/page-route-animation)
- [Widget 的物理模拟动画效果](/docs/cookbook/animation/physics-simulation)
- [Container 里的动画渐变效果](/docs/cookbook/animation/animated-container)
- [Widget 的淡入淡出效果](/docs/cookbook/animation/opacity-animation)


## Design
- [在屏幕上添加一个 Drawer](/docs/cookbook/design/drawer)
- [显示 snackbars](/docs/cookbook/design/snackbars)
- [以 package 的方式使用字体](/docs/cookbook/design/package-fonts)
- [根据屏幕方向更新界面](/docs/cookbook/design/orientation)
- [使用自定义字体](/docs/cookbook/design/fonts)
- [使用 Themes 统一颜色和字体风格](/docs/cookbook/design/themes)
- [使用 tabs](/docs/cookbook/design/tabs)


## Effects
- [Create a download button](/docs/cookbook/effects/download-button)
- [Create a nested navigation flow](/docs/cookbook/effects/nested-nav)
- [Create a photo filter carousel](/docs/cookbook/effects/photo-filter-carousel)
- [Create a scrolling parallax effect](/docs/cookbook/effects/parallax-scrolling)
- [Create a shimmer loading effect](/docs/cookbook/effects/shimmer-loading)
- [Create a staggered menu animation](/docs/cookbook/effects/staggered-menu-animation)
- [Create a typing indicator](/docs/cookbook/effects/typing-indicator)
- [创建一个点击展开的 FAB](/docs/cookbook/effects/expandable-fab)
- [Create gradient chat bubbles](/docs/cookbook/effects/gradient-bubbles)
- [Drag a UI element](/docs/cookbook/effects/drag-a-widget)


## Forms
- [构建一个有验证判断的表单](/docs/cookbook/forms/validation)
- [文本框的创建和设定](/docs/cookbook/forms/text-input)
- [焦点和文本框](/docs/cookbook/forms/focus)
- [响应文本框内容的更改](/docs/cookbook/forms/text-field-changes)
- [获取文本框的输入值](/docs/cookbook/forms/retrieve-input)


## Gestures
- [添加点按涟漪效果 (Material Design)](/docs/cookbook/gestures/ripples)
- [捕获和处理点击动作](/docs/cookbook/gestures/handling-taps)
- [实现「滑动清除」效果](/docs/cookbook/gestures/dismissible)


## Images
- [显示网络上的远程图片](/docs/cookbook/images/network-image)
- [占位符和网络图片淡入](/docs/cookbook/images/fading-in-images)
- [使用缓存图片](/docs/cookbook/images/cached-images)


## Lists
- [创建一个网格列表](/docs/cookbook/lists/grid-lists)
- [创建一个水平滑动的列表](/docs/cookbook/lists/horizontal-list)
- [创建拥有不同列表项的列表](/docs/cookbook/lists/mixed-list)
- [在列表顶部放置一个浮动的 app bar](/docs/cookbook/lists/floating-app-bar)
- [基础列表](/docs/cookbook/lists/basic-list)
- [长列表的处理](/docs/cookbook/lists/long-lists)


## Maintenance
- [把报错信息通过服务上传](/docs/cookbook/maintenance/error-reporting)


## Navigation
- [跨页面切换的动效 Widget (Hero animations)](/docs/cookbook/navigation/hero-animations)
- [导航到一个新页面和返回](/docs/cookbook/navigation/navigation-basics)
- [导航到对应名称的 routes 里](/docs/cookbook/navigation/named-routes)
- [给特定的 route 传参](/docs/cookbook/navigation/navigate-with-arguments)
- [从一个页面回传数据](/docs/cookbook/navigation/returning-data)
- [传递数据到新页面](/docs/cookbook/navigation/passing-data)


## Networking
- [删除网络数据](/docs/cookbook/networking/delete-data)
- [获取网络数据](/docs/cookbook/networking/fetch-data)
- [发起 HTTP 认证授权请求](/docs/cookbook/networking/authenticated-requests)
- [在后台处理 JSON 数据解析](/docs/cookbook/networking/background-parsing)
- [发送网络数据](/docs/cookbook/networking/send-data)
- [更新网络数据](/docs/cookbook/networking/update-data)
- [发起 WebSockets 请求](/docs/cookbook/networking/web-sockets)


## Persistence
- [用 SQLite 做数据持久化](/docs/cookbook/persistence/sqlite)
- [文件读写](/docs/cookbook/persistence/reading-writing-files)
- [存储键值对数据](/docs/cookbook/persistence/key-value)


## Plugins
- [视频的播放和暂停](/docs/cookbook/plugins/play-video)
- [使用 Camera 插件实现拍照功能](/docs/cookbook/plugins/picture-using-camera)


## Testing

### Integration
- [An introduction to integration testing](/docs/cookbook/testing/integration/introduction)
- [Performance profiling](/docs/cookbook/testing/integration/profiling)

### Unit
- [单元测试介绍](/docs/cookbook/testing/unit/introduction)
- [使用 Mockito 模拟依赖关系](/docs/cookbook/testing/unit/mocking)


### Widget
- [An introduction to widget testing](/docs/cookbook/testing/widget/introduction)
- [Find widgets](/docs/cookbook/testing/widget/finders)
- [Handle scrolling](/docs/cookbook/testing/widget/scrolling)
- [Tap, drag, and enter text](/docs/cookbook/testing/widget/tap-drag)

如果想看到更多的教程，可以参考我们的 [社区中文教程页面](/community/tutorials)。