...
1. Install it
   ...
   * From VS Code: Click 'Get Packages' located in right side of the action
     ...
     在 VS Code 中点击位于 `pubspec.yaml` 文件顶部操作功能区右侧的 'Packeage Get'
...
当使用简写形式 `plugin1:` 将 package 添加到 `pubspec.yaml` 时，这将被解释为 `plugin: any`，即 package 的任何版本都可以被使用。为了确保在更新 package 的时候你的应用不会奔溃，我们建议使用以下格式之一来指定版本范围：
...
即使未在 Pub 上发布，也可以使用 package。对于不用于公开发布的私有插件，或者尚未准备好发布的 package，可以使用其他依赖选项。
