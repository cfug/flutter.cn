...
例如，你可能会为一个 widget 添加动画，让它看起来就像安着弹簧，或是在随重力下落。
...
为了调整对齐方式，请使用 [MediaQuery][] 来获得 Widget 的大小，然后除以 2。（这会将“拖动的像素”单位转为 [Align][] 使用的坐标。）然后，将 `Align` widget 的 `alignmnt` 属性设为 `_dragAlignment`。
