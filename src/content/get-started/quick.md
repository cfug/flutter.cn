...
description: 使用 VS Code 等基于 OSS 的编辑器在你的设备上设置 Flutter，并开始开发你的第一个多平台应用！
showBanner: false
sitemap: false
---
...
了解如何使用 VS Code 等基于 OSS 的编辑器
来设置 Flutter 开发环境，并体验 Flutter 的开发流程。
...
如果你之前使用过 Flutter 进行开发，
或者你更喜欢使用其他编辑器或 IDE，
可以按照[自定义设置说明][custom setup instructions]进行操作。
...
本页面的说明适用于在 **Windows**{:.selected-os-text} 设备上安装和体验 Flutter。

If you'd like to follow the instructions for a different OS,
please select one of the following.

如果你想按照其他操作系统的说明进行操作，请选择以下选项之一。
...
为了最顺利地设置 Flutter，请先安装以下工具。
 ...
 1. <h3>Download and install Visual Studio Code</h3>
    ...
    要快速安装 Flutter，然后编辑和调试你的应用，
    请[安装和设置 Visual Studio Code][vscode-install]。
    ...
    你也可以安装和使用任何其他支持 VS Code 扩展的基于 Code OSS 的编辑器。
    如果你选择这样做，在本文的其余部分，
    假设 VS Code 指的是你选择的编辑器。
...
{: .steps .macos-only}
 ...
 1. <h3>Install Git for Windows</h3>
    ...
    如需安装或排查 Git 问题的帮助，请参阅 [Git 文档][git-install]。
...
请按照以下步骤使用 VS Code 安装和设置 Flutter。
...
如果你更喜欢手动安装 Flutter，
请按照[手动安装 Flutter][Install Flutter manually]中的说明进行操作。
...
:::
 ...
 1. <h3>Launch VS Code</h3>
    ...
    如果 VS Code 尚未打开，请使用 Spotlight 搜索它，
    或从安装目录手动打开它。
 ...
 1. <h3>Add the Flutter extension to VS Code</h3>
    ...
    要将 Dart 和 Flutter 扩展添加到 VS Code，
    请访问 [Flutter 扩展的应用商店页面][flutter-vscode]，
    然后点击 **Install**。
 ...
 1. <h3>Install Flutter with VS Code</h3>
    ...
    1. Click **Clone Flutter**.
       ...
       在下载 Flutter 时，VS Code 会显示以下弹出通知：
       ...
       此下载需要几分钟时间。
       如果你怀疑下载已挂起，请点击 **Cancel**，然后重新开始安装。
 ...
 1. <h3>Troubleshoot installation issues</h3>
    ...
    如果你在安装过程中遇到任何问题，
    请查看 [Flutter 安装故障排除][troubleshoot]。
...
是时候创建一个应用并体验 Flutter 开发了！
 ...
 1. <h3>Create a new Flutter app</h3>
    ...
    1. Choose the **Application** template.
       ...
       VS Code 应该会提示你 **Which Flutter template?**。
    ...
    1. Enter a name for your app.
       ...
       VS Code 应该会提示你为新应用输入名称。
       输入 `trying_flutter` 或类似的 `lowercase_with_underscores` 格式的名称。
 ...
 1. <h3>Run your app on the web</h3>
    ...
    1. Run or start debugging your app.
       ...
       **Start Debugging** 或按 <kbd>F5</kbd>。
       ...
       使用 `flutter run` 来构建和启动你的应用，
       然后一个新的 Chrome 实例应该会打开并开始运行你新创建的应用。
 ...
 1. <h3>Try hot reload</h3>
    ...
    Flutter 通过**有状态热重载**提供快速开发周期，
    能够在不重启或丢失应用状态的情况下重新加载正在运行的应用代码。
    ...
    1. In the running app, try adding to the counter a few times by
       ...
       在运行的应用中，尝试通过点击
       ![increment (+)][increment-button]{: .text-icon} 按钮来增加计数器几次。

    1. With your app still running, make a change in the `lib/main.dart` file.

       在应用仍在运行的情况下，在 `lib/main.dart` 文件中进行更改。
    ...
    1. Try clicking the
       ...
       再次尝试点击 ![increment (+)][increment-button]{: .text-icon} 按钮。
       注意值现在是减少而不是增加。
 ...
 1. <h3>Explore the Flutter sidebar</h3>
    ...
    1. Open the Flutter sidebar in VS Code.
       ...
       可以使用 VS Code 侧边栏中的 Flutter ![Flutter logo][]{: .text-icon} 按钮打开它，
    ...
    1. In the widget inspector, try clicking the top-level `MyHomePage` widget.
       ...
       应该会打开它的属性和布局视图，
       VS Code 编辑器应该会导航到并聚焦包含该 widget 的代码行。
