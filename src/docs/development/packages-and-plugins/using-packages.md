...
为了强制使用版本为 `0.4.3` 的 `url_launcher`，
...
`flutter pub get`（IntelliJ 中的 'Packages Get'），
...
请运行 `flutter packages upgrade`（Intellij 中的 `flutter packages upgrade`）。
...
* **Path** dependency: A Flutter app can depend on a plugin via a file system
  ...
  **Path** 依赖，Flutter 应用可以通过文件系统 `path:` 依赖而依赖于插件。
  路径可以是相对的，也可以是绝对的。
...
* **Git** dependency: You can also depend on a package stored in a Git
  ...
  **Git** 依赖：你也可以依赖存储在 Git 仓库中的 package，
...
* **Git** dependency on a package in a folder: By default Pub assumes the
  ...
  **Git** 依赖于文件夹中的 package：默认情况下，
  Pub 假定 package 位于 Git 仓库的根目录。
...
（我们将这些 package 称为**插件**）。它同时支持 Android 和 iOS。
