...
请在下方选择你的 AI Agent，来获取安装 Flutter 和 Dart 官方 Agent skills 的说明。
...
如果你使用 Claude Code，请安装 Flutter 官方插件。
浙江同时安装 Flutter 和 Dart Agent skills，
以及 [Dart 和 Flutter MCP server](/ai/mcp-server)：

1.  Add the Flutter marketplace for Claude plugins:

    添加 Flutter 插件市场，以支持 Claude 插件：
...
默认情况下，兼容的 AI Agent 会在项目工作区的 `.agents/skills` 目录中发现 Agent skills。
...
`flutter/skills` 仓库已更名为 `flutter/agent-plugins`。
如果你之前使用 `flutter/skills` 安装过 skills，
请将命令更新为 `flutter/agent-plugins`。
:::
...
执行这些命令会自动创建 `.agents/skills` 目录，
并将所需的 skills 下载到你的项目中。
...
[Flutter agent-plugins 仓库](https://github.com/flutter/agent-plugins)。
...
将 skills 添加到项目后，可以让 AI Agent 查看你已经安装的 skills。
