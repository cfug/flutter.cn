...
decoder.
在 Flutter 中基本的 JSON 编码是十分容易的。Flutter 有一个内置的 `dart:convert` 的库，这个库包含了一个简单的 JSON 编码器和解码器。
...
在序列化 JSON 数据的例子中，这些必须的依赖的最新版本可以在下面 [the pubspec file](https://raw.githubusercontent.com/dart-lang/json_serializable/master/example/pubspec.yaml) 上查看。
