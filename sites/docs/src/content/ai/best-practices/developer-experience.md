...
不过，借助更好的提示词，示例应用用 Gemini 2.5 Pro 实现了大部分功能，用 Gemini 3 Pro Preview 做了最后润色。从两个模型获得最佳结果的流程相同：
...
（中文大意：在项目根 plans 文件夹创建 requirements.md；应用为 flutter/demos 下的开源示例，演示 Flutter、Firebase AI Logic 与 Gemini 构建可解小型填字游戏的智能体工作流；附更多描述与示例截图；开始前请先提问。）

该应用将作为开源示例托管在 GitHub 的 flutter/demos 目录中，旨在演示如何使用 Flutter、Firebase AI Logic 和 Gemini 构建能解决小型填字游戏（尺寸小于 10×10）的智能体工作流……还有更多应用描述以及示例谜题截图……
在开始之前，如有任何问题请先提问。
...
这条提示词，加上少量问答、人工编辑以及编码过程中的一些更新，产出了[需求文件][requirements]。
...
DRY（Don't Repeat Yourself，不要重复自己）——通过提取共享工具与模块消除重复逻辑。

关注点分离——每个模块应处理一项明确职责。

单一职责原则（SRP）——每个类/模块/函数/文件应只有一个变更理由。

清晰抽象与契约——通过小而稳定的接口暴露意图，隐藏实现细节。

低耦合、高内聚——保持模块自包含，尽量减少跨模块依赖。

可扩展性与无状态——设计可水平扩展的组件，尽可能优先无状态服务。

可观测性与可测试性——内置日志、指标、追踪，并确保组件可进行单元/集成测试。

KISS（Keep It Simple, Sir，保持简单）——尽可能保持方案简单。

YAGNI（You're Not Gonna Need It，你不需要它）——避免投机性复杂度和过度工程。
...
GEMINI.md 文件会加载到你用 Gemini 创建的每条新提示词中；它提供你希望 Gemini 在任何活动中记住的规则集。Gemini 运行在一个空的 Flutter 应用项目中，因此 `/init` 命令记录了如何构建、测试和运行，这在编码过程中很有用。
...
很好。我想和你一起完成设计，并写入 plans 文件夹中的 design.md。请使用 @GEMINI.md 和 @requirements.md 作为输入。在开始之前，如有任何问题请先提问。
...
请阅读 @specs 文件夹中的文件，并在同一文件夹创建对应的 tasks.md，列出表示该应用功能的一组任务与子任务。将顶层任务安排为运行应用中用户可见的最小新增功能，每完成一个顶层任务推进一步。每个顶层任务应包含创建并运行测试、以及用当前功能描述更新 @README.md 的子任务。在开始之前，如有任何问题请先提问。
...
这一切都在写任何代码之前完成。你不必拆成独立文件，但通过认真考虑需求、设计和任务拆解，你是在帮助智能体提供符合你预期的结果。这称为「规格驱动开发（Spec-Driven Development）」，目前是我们所知的将流程从「凭感觉编码（vibe coding）」升级为「AI 辅助软件开发」的最佳方式。
...
使用 git diff 找出新代码，对照此处列出的架构原则检查：@GEMINI.md。对重要改进提出建议。
