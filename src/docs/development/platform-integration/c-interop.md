...
Flutter 移动版可以使用 [dart:ffi][] 库来调用本机的 C API。
_FFI_ 代表[_外部功能接口_][FFI]。
类似功能的其他术语包括 _本机接口和语言绑定_。
...
在库或程序可以使用 FFI 库绑定到本机代码之前，必须确保本机代码已加载并且其符号对 Dart 可见。
本页主要介绍在 Flutter 插件或应用程序中编译、打包和加载本机代码。
...
本教程演示如何在 Flutter 插件中捆绑 C/C++ 源代码，并使用 Android 和 iOS 上的 Dart FFI 库绑定它们。
在本示例中，您将创建一个实现 32 位加法的 C 函数，然后通过名为 "native_add" 的 Dart 插件公开它。
...
{{ site.alert.note }}
  ...
  目前 dart:ffi 库还在 [beta 中][ffi issue]，并且可能一直存在重大修改。
...
静态链接中的符号表可以使用 `DynamicLibrary.executable` 或 `DynamicLibrary.process`.
...
相比之下，动态链接库则分布在应用程序中的单独文件或文件夹中，并按需加载。
在Android上，动态链接库作为一组 `.so`(ELF) 文件分发，每个架构对应一个。
在iOS上，它是作为 `.framework` 文件夹分发的。
...
Dart dev 频道中的 API 代码已经可用：
[Dart Api 文档][Dart API reference documentation].
...
您可以将源代码添加到 ios 文件夹，因为 CocoaPods 不允许在 podspec 文件上方包含源代码，但是 Gradle 允许您指向ios文件夹。
iOS 和 Android 不需要使用相同的源代码；当然，您也可以将特定于 Android 的源代码添加到 Android 文件夹并修改 CMakeLists.txt 文件。
...
FFI 库只能与 C 符号绑定，因此在 C++ 中，这些符号必须标记为 `extern C`。还应该添加属性来指示符号是从 DART 引用的，以防止链接器在链接时间优化期间丢弃符号。
...
例如，创建一个 c++ 文件，路径为： `ios/Classes/native_add.cpp`。
（请注意，模板已经为您创建了此文件。）在项目的根目录命令行中执行：
...
在 iOS，你需要告诉 Xcode 如何静态连接这个文件

 1. In Xcode, open `Runner.xcworkspace`.
    
    在 Xcode中，打开 `Runner.xcworkspace`。
...
在 Android 中，你需要创建一个 `CMakeLists.txt` 文件去定义应该如何编译源文件，
然后还要告诉 Gradle 如何去定位它们。
在项目根目录下，运行如下代码：
...
最后，添加一个 `externalNativeBuild` 到你的 `android/build.gradle`文件中。
示例如下：
...
首先，你需要创建一个 `DynamicLibrary` 来处理本机代码。
这一步在iOS和Android之间有所不同：
...
请注意，在Android上，本机库的名称是定义在 `CMakeLists.txt`中（见上文），但在iOS上它使用插件的名称。

With a handle to the enclosing library,
you can resolve the `native_add` symbol:

使用库的的句柄，你可以解析 `native_add` 符号：
...
现在，你可以调用它了。
在自动生成的 example 项目(`example/lib/main.dart`)中演示它。
...
您还可以使用 [`DynamicLibrary.open`][] 来限制符号解析的范围，
但目前还不清楚苹果的审查程序如何处理这一问题。

Symbols statically linked into the application binary
can be resolved using [`DynamicLibrary.executable`][] or
[`DynamicLibrary.process`][].

静态链接到应用程序二进制文件的符号可以使用 [`DynamicLibrary.executable`][] 或 [`DynamicLibrary.process`][].
...
1. In Xcode, open `Runner.xcworkspace`.
   ...
   在 Xcode 中，open `Runner.xcworkspace`。
...
第一方本机库可以作为源文件或（签名的）`.framework` 文件包含在内。
也可能包括静态链接的档案，但它需要测试。
...
1. If a properly signed `Framework` file is present,
   ...
   如果存在签名正确的`Framework` 文件，请打开`Runner.xcworkspace`。
...
要添加一个闭源的库到 [Flutter mac 桌面][Flutter macOS Desktop] 应用
请按照如下说明：

1. Follow the instructions for Flutter desktop to create a Flutter desktop app.

   按照 Flutter 桌面的说明创建 Flutter 桌面应用程序。
...
1. Open the `yourapp/macos/Runner.xcworkspace` in XCode.
   ...
   1. Drag your precompiled library (`libyourlibrary.dylib`) into `Runner/Frameworks`.
      ...
      拖动你的预编译的 `libyourlibrary.dylib` 到你的 `Runner/Frameworks`。
   ...
   2. Click `Runner` and go to the `Build Phases` tab.
      ...
      3. Under `Link Binary With Libraries`, set status to `Optional`. (We use dynamic linking, no need to statically link.)
         ...
         在 `Link Binary With Libraries` 下，设置状态为 `Optional`。（我们使用动态连接，不需要静态链接）

   3. Click `Runner` and go to the `General` tab.

      点击 `Runner` 然后进入 `General` 标签。
...
1. Edit `lib/main.dart`.
   ...
   1. Use `DynamicLibrary.open('libyourlibrary.dylib')` to dynamically link to the symbols.
      ...
      使用 `DynamicLibrary.open('libyourlibrary.dylib')` 来动态连接符号表。
...
去创建一个包含 C/C++/Objective-C _and_ Dart 代码的 Flutter插件，
你需要如下步骤：
...
要创建包含 DART 源代码的 Flutter 插件，
但是以二进制形式分发 C/C++ 库，请使用以下说明：
...
**不要**将此插件（或任何包含二进制代码的插件）上载到 pub.dev。
相反，应该从可信的第三方下载此插件，如 CocoaPods 示例所示。
...
使用如下步骤：
 ...
 1. Find the desired library in the [Android NDK Native APIs][]
    ...
    在 Android 文档的 [Android NDK Native api][] 列表中找到所需的库。它列出了稳定的本地 API。
...
在源代码或二进制形式中包含本机代码的过程与应用程序或插件的过程相同。
...
遵循安卓文档[添加C和C++代码到项目][Add C and C++ code to your project]
来添加本地代码和对本机代码工具链的支持（CMake 或 `ndk-build`）。
...
要创建包含 Dart 源代码的 Flutter 插件，但是以二进制形式分发 C/C++ 库，请使用以下说明：
...
[Android指南][Android guidelines] 通常建议分发未压缩的本机共享对象，因为这样实际上可以节省设备空间。
共享对象可以直接从APK加载，而不是将它们解压到设备上的临时位置然后再加载。
APK是在传输过程中额外打包的 - 这就是为什么您应该查看下载文件尺寸。
...
Flutter apk 文件默认情况下不遵循这些指导原则和压缩 `libflutter.so` 和 `libapp.so`，
这导致 APK 体积更小，但设备体积更大。
...
来自第三方的共享库可以使用其 `AndroidManifest.xml` 中的 `android:extractNativeLibs="true"` 更改此默认设置，
停止压缩 `libflutter.so`、`libapp.so` 和任何用户添加的共享库。
要重新启用压缩，请按照如下方式重写您的 `your_app_name/android/app/src/main/AndroidManifest.xml`。
...
2. Change from **All Symbols** to **Non-Global Symbols**.
   ...
   从 **All Symbols** 修改为 **Non-Global Symbols**。
