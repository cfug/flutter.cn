...
手动安装更新：
 ...
 1. 点击侧边栏的扩展按钮
 1. 如果 Flutter 扩展显示有可用更新，点击更新按钮，然后重载。
...
通过 Flutter 入门应用模板新建 Flutter 项目：
 ...
 1. 选择 **Flutter: New Project** 命令然后按 `Enter`。
 1. 输入你想要的**项目名**。
 1. 选择**项目地址**。
...
### 无断点运行
 1. 在 IDE 主窗口点击 **Debug > Start Without Debugging**，或者按 `Ctrl`+`F5`。
    状态栏变橙色说明你正处于调试模式。<br>
...
### 断点运行
 ...
 1. 如果需要，在源代码中设置断点。
 1. 在 IDE 主窗口点击 **Debug > Start Debugging** 或按 `F5`。
    * 左侧的**调试侧边栏**显示堆栈帧和变量。
