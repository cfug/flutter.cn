...
{{site.alert.secondary}}
  ...
  `FlutterViewController` 依附于 `FlutterEngine`，给 Flutter 传递 UIKit 的输入事件，
  并展示被 `FlutterEngine` 渲染的每一帧画面。
{{site.alert.end}}
...
创建 `FlutterEngine` 的合适位置取决于您的应用。作为示例，我们将在应用启动的 app delegate
中创建一个 `FlutterEngine`， 并作为属性暴露给外界。
...
防止被 [tree-shaking](https://en.wikipedia.org/wiki/Tree_shaking) 优化掉，
而没有编译。
...
通过 [撰写双端平台代码](/docs/development/platform-integration/platform-channels)，
