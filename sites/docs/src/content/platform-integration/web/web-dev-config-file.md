...
Flutter Web 包含开发服务器，默认在 `localhost` 域通过 HTTP 在随机分配的端口上提供应用。
命令行参数可快速修改服务器行为，本文档侧重更结构化的方式：
通过集中的 `web_dev_config.yaml` 文件定义服务器行为。
该配置文件可自定义主机、端口、HTTPS 设置与代理规则，确保开发环境一致。
...
在 Flutter 项目根目录添加 `web_dev_config.yaml` 文件。
若尚未创建 Flutter 项目，请参阅 [使用 Flutter 构建 Web 应用][Building a web application with Flutter] 入门。
...
*   A request to `/data/2023/` is
    ...
    对 `/data/2023/` 的请求会转发到 `http://localhost:3000/report/2023`，
    因为 `replace: "/report/"` 会替换 `/data/` 前缀。
...
*   A request to `/products/item/123` is
    ...
    对 `/products/item/123` 的请求会转发到 `http://localhost:4000/item/123`，
    因为 `replace: ""` 会用空字符串替换 `/products/` 前缀从而将其移除。
...
*   A request to `/users/123/` matches the first rule exactly,
    ...
    对 `/users/123/` 的请求完全匹配第一条规则，
    因此转发到 `http://localhost:5000/users/123/`。
...
*   A request to `/api/v1/users/profile/` starts with the second rule path
    ...
    对 `/api/v1/users/profile/` 的请求匹配第二条规则，
    因此转发到 `http://localhost:4000/users/profile/?apiVersion=v1`。
