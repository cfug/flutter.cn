...
我们一直在不断努力降低 Dart 代码的应用体量和内存占用量。在大型 Flutter 应用中，经过 AOT 编译 Dart 程序的元数据的内部结构可能要占用非常可观的内存。这些元数据的存在大多是为了实现热重载、交互式调试，以及格式化可读堆栈轨迹等功能，这些功能在需要部署的应用中从不会用到。过去几年来，我们一直在重构 Dart 原生运行时环境，以便尽可能多地消除这种开销。其中一些改进适用于所有以版本模式构建的 Flutter 应用，而有些则需要使用 [--split-debug-info](https://dart.cn/docs/perf/app-size#reducing-app-size) 标志将 AOT 编译应用中的调试信息拆分出来，从而放弃可读的堆栈轨迹。
...
如果对您来说应用体量和内存占用量比较重要，可以使用 [--split-debug-info](https://dart.cn/docs/perf/app-size#reducing-app-size) 标志省略调试信息。请注意，一旦这么做，您需要使用 [symbolize 命令](https://dart.cn/docs/deployment/obfuscate#reading-an-obfuscated-stack-trace) 来重新使堆栈轨迹可读。
