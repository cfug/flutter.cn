...
description: 了解如何使用适用于 Dart 和 Flutter 的 Antigravity CLI。
ai-translated: true
---
...
如果你之前使用过 Gemini CLI 或适用于 Flutter 的 Gemini CLI 扩展，
...
当你第一次在终端中运行 `agy` 时，
该工具会检查是否存在现有的 Gemini 配置文件
（例如 `~/.gemini/config/mcp_config.json` 或旧版环境变量）。
如果找到，该工具会询问你是否要自动迁移你的设置、
偏好和 API 配置。
...
并将其配置复制到你的活动 Antigravity 配置文件中。
...
与旧版 Gemini CLI 工具一样，
Antigravity CLI 遵循存储在你工作区目录中的自定义开发指南和配置：
...
- **Local rules**: You can place rules files in your project directory
  ...
  **本地规则**：你可以在项目目录中放置规则文件
  ...
  以指导代理遵循特定的编码风格指南或架构模式。
  请注意，Antigravity CLI 也支持与旧版 `GEMINI.md` 文件
  的向后兼容性；
...
- **Global configuration**: Antigravity CLI stores global settings
  ...
  **全局配置**：Antigravity CLI 将全局设置和配置的 MCP 服务器存储在 `~/.antigravity/` 中
