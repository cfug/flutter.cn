...
许多 `app` 都有内容列表，从电子邮件客户端到音乐 `app` 等等。我们期望使用集成测试来验证列表中的内容，我们需要一种方法去滚动列表来查找特定的项。
...
为了使滚动列表通过集成测试，我们可以使用 [`flutter_driver`](https://github.com/cfug/flutter.cn/blob/master/src/docs/cookbook/testing/integration/%7B%7Bsite.api%7D%7D/flutter/flutter_driver/flutter_driver-library.html) 这个 `package` 中的 [`FlutterDriver`](https://github.com/cfug/flutter.cn/blob/master/src/docs/cookbook/testing/integration/%7B%7Bsite.api%7D%7D/flutter/flutter_driver/FlutterDriver-class.html) 类。
...
在本章节，我们将学习如何滚动列表并验证列表中显示的是特定的 `Widget` ，并讨论不同方法的优缺点。如果您刚刚开始进行集成测试，请阅读 [`集成测试介绍`](https://github.com/cfug/flutter.cn/blob/master/docs/cookbook/testing/integration) 获取更多信息。
...
在本章节，我们创建一个带有长列表的 `app` 。为了能够在本章节中专注于测试，我们将使用在 [长列表使用](https://github.com/cfug/flutter.cn/blob/master/docs/cookbook/lists/long-lists) 章节中创建的 `app` 。如果你不确定如何处理内容列表，请自行查看相关章节的介绍。
...
现在，我们可以编写我们的测试用例了！在这个例子中，我们需要滚动列表并校验特定的列表项是否存在于列表中。[`FlutterDriver`](https://github.com/cfug/flutter.cn/blob/master/src/docs/cookbook/testing/integration/%7B%7Bsite.api%7D%7D/flutter/flutter_driver/FlutterDriver-class.html) 类为我们的滚动列表提供了三个方法：
