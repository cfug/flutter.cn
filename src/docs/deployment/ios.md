...
这个教材提供如何逐步将 Flutter App 发布到 [App Store][appstore] 和 [TestFlight][testflight] 的说明。
...
在开始发布你的 App 的进程之前，确保你已经看过了 Apple 的 [App 审核指南][appreview]。
...
想要发布你的 App 到 App Store，你需要注册 [Apple 开发者项目][devprogram]。你可以在苹果的 [选择一种会员类型][devprogram_membership] 中查看到关于多种不同会员类型的选择。
...
[App Store Connect][appstoreconnect]（曾经的 iTunes Connet）是你将会管理你的 App 生命周期的地方。你将会定义你的 App 的名称和描述以及截图，设置价格，并管理发布到 App Store 和 Testflight。
...
注册你的 App 需要两步：注册一个唯一的 Bundle ID，并在你的 App Store Connect 中创建一个 App。

For a detailed overview of App Store Connect, see the [App Store Connect][appstoreconnect_guide]
guide.

关于更多 App Store Connect 的细节，查看 [App Store Connect][appstoreconnect_guide] 指南。
...
想要获取更多信息，可以看 [添加一个 App 到你的账户][appstoreconnect_guide_register]。
...
在这一步，你需要在 Xcode 工作空间检查绝大多数重要设置。关于更多的步骤和描述，查看 [为 App 分发做准备][distributionguide_config]。
...
在这一步，你将创建一个构建归档，并上传你的构建到 App Store Connect。
...
在开发过程中，你将会使用 *debug* 构建来完成构建、调试并测试。当你准备好通过 App Store 或 TestFlight 交付你的 App 给用户时，你需要准备一个 *release* 构建。
...
关于更多信息，查看 [使用 TestFlight 分发一个应用][distributionguide_testflight]。
...
当你准备发布你的 App 到这个世界是，跟随下面的步骤，来提交你的 App 去审核，并将其发布到 App Store。
