...
『体积信息』包含 dart 代码、原生代码和非代码部分（比如应用包，资产和字体）。一个『体积信息』文件包含你应用的所有图片数据。
...
AOT 编译器不用于 debug 生成, debug 模式是JIT编译的）。
...
把它放在应用程序大小分析的上下文中，想象一下`package:a` 导入了 `package:b` 和 `package:c`，
...
此文件包含整个应用程序的大小信息（本机代码、Dart 代码、资产、字体等），
