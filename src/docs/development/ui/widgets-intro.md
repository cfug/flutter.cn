...
[StatefulWidget]({{api}}/widgets/StatefulWidget-class.html)，这取决于 widget 是否管理组件的状态。
组件的主要工作是是实现 [build](https://api.flutter.dev/flutter/widgets/StatelessWidget/build.html) 方法。
该方法根据其他更低级别的 widget 来构建自身。框架会逐一构建这些组件直到最底层的代表 [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) 的组件创建完成， [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html) 它用来计算和描述组件的几何形状的。
 ...
 * [Container]({{api}}/widgets/Container-class.html): The
   ...
   [Container](https://api.flutter.dev/flutter/widgets/Container-class.html): [Container](https://api.flutter.dev/flutter/widgets/Container-class.html) 组件可以用来创建一个可见的矩形元素。容器可以使用 [BoxDecoration](https://api.flutter.dev/flutter/painting/BoxDecoration-class.html) 来进行装饰，如背景，边框，或阴影等。[Container](https://api.flutter.dev/flutter/widgets/Container-class.html) 还可以设置外边缘，内边缘尺寸的约束条件。此外容器可以使用矩阵在三维空间进行转换。
...
确保将属性 `uses-material-design: true` 放在 `pubspec.yaml` 文件入口的 `flutter` 部分，
这样就可以使用预定义的 [Material icons](https://design.google.com/icons/)。
...
很多 Material Design 组件需要在 [MaterialAPP](https://api.flutter.dev/flutter/material/MaterialApp-class.html)
 中合理展现，这样就可以访问它的默认主题。因此我们可以以 [MaterialAPP](https://api.flutter.dev/flutter/material/MaterialApp-class.html) 的方式运行应用程序。
...
上例中的 MyAppBar 组件创建了一个高为 56 dp的 [Container](https://api.flutter.dev/flutter/widgets/Container-class.html)，
左右内边缘为 8 dp。在容器里面，MyAppBar 以[水平](https://api.flutter.dev/flutter/widgets/Row-class.html)
布局来布局子元素。中间子元素也就是标题组件，设为 [Expanded](https://api.flutter.dev/flutter/widgets/Expanded-class.html)
 填充，也就是它要填充剩余空间。你可以定义多个 [Expanded](https://api.flutter.dev/flutter/widgets/Expanded-class.html)
 子元素，然后用 [flex](https://api.flutter.dev/flutter/widgets/Expanded-class.html) 属性设置每个子元素所占的比进行 [Expanded](https://api.flutter.dev/flutter/widgets/Expanded-class.html)。
...
MyScaffold 以垂直方式来布局它的子元素组件。在顶部放了一个 MyAppBar 组件实例，并把 [Text](https://api.flutter.dev/flutter/widgets/Text-class.html) 组件传给它来作为应用的标题。把组件作为参数传递给其他的组价是一个很强大的技术，它可以让你以各种方式创建一些
可重用的通用组件。最后，MyScaffold 使用 [Expanded](https://api.flutter.dev/flutter/widgets/Expanded-class.html)
...
既然我们已经从 myAppBar 和 MyScaffold 说到 material.dart 中的 [AppBar](https://api.flutter.dev/flutter/material/AppBar-class.html) 和 [Scaffold](https://api.flutter.dev/flutter/material/Scaffold-class.html) 组件，我们可以来看看更多 Material 风格的效果。例如，应用的标题栏有阴影，并且标题组件自动继承正确的样式。我们也加了一个悬浮按钮看起来更美观。
...
通过系统来处理一些用户交互。创建可交互的应用程序的第一步就是检测输入的手势。我们通过下面的一个简单的按钮实例来看看它的工作机制：
...
[GestureDetector](https://api.flutter.dev/flutter/widgets/GestureDetector-class.html) 组件在界面上是看不到的，但是它能识别用户的手势。当用户点击 [Container](https://api.flutter.dev/flutter/widgets/Container-class.html)，[GestureDetector](https://api.flutter.dev/flutter/widgets/GestureDetector-class.html) 会调用 [onTap](https://api.flutter.dev/flutter/widgets/GestureDetector-class.html#onTap) 回调，此时会在控制台上输出一行信息。你可以使用 [GestureDetector](https://api.flutter.dev/flutter/widgets/GestureDetector-class.html) 组件检测各种输入手势，包括点击，拖拽，缩放。
...
很多组件使用 [GestureDetector](https://api.flutter.dev/flutter/widgets/GestureDetector-class.html) 来为其他组提供可选的回调，例如
...
到目前为止，我们只使用了无状态组件。无状态组件接收的参数来自于它的父组件，并将参数存为 [final](https://www.dartlang.org/guides/language/language-tour#final-and-const)
 成员变量。当需要 [build](https://api.flutter.dev/flutter/widgets/StatelessWidget/build.html) 子组件时，就是用这些存储的变量为创建的组件生成新的参数。
...
为了创建更复杂的经验，比如对用户的输入有更复杂的交互，应用通常会维护一些状态。Flutter使用有状态组件来实现这一想法。有状态组件会产生一些状态对象来持有这些状态。参考使用上文提到过的[按钮](https://api.flutter.dev/flutter/material/RaisedButton-class.html)来实现的一个简单示例：
...
组件是临时对象，用于展现应用当前的状态。另一方面，状态对象在调用和[创建](https://api.flutter.dev/flutter/widgets/State/build.html)
之间是持久的，以此来存储信息。
...
比如，一个 widget 可能呈现一个复杂的用户界面来收集特定的信息，比如日期，位置等，而另一个组件就用这些信息来改变整体的布局显示。
...
在 Flutter 中，Stateless widget 通过回调得到状态改变的通知，然后当前状态又通知给其他 widget 用于显示。下面的复杂小示例展示了在实践中它的工作机制：
...
注意到我们是如何创建两个无状态组件，分别用来显示数量(CounterDisplay)和改变数量(CounterIncrementor)。尽管最后的结果和之前的例子是一样的，
这种责任分离可以保证父组件简单，同时各个组件也可以封装更复杂的功能。
...
下面是一个完整的示例，将上面所提到一些概念结合起来。一个购物应用提供了各种商品来售卖，还需要在购物车中添加一些想购买的物品，首先定义一个展现类，`ShoppingListItem`:
...
`ShoppingListItem` 类继承自无状态组件，我们就以 stateless 的模式来编写这个 Widget。它将构造函数中接受到的值存储在 [final](https://www.dartlang.org/guides/language/language-tour#final-and-const) 成员变量中，这些值在[构建](https://api.flutter.dev/flutter/widgets/StatelessWidget/build.html)
函数的时候会使用到。例如，`inCart` 有两种样式，一个是使用当前主题的样式，一个是使用灰色。
...
当用户点击列表中的一项，组件不会直接改变 `inCart` 的值，而是通过父组件调用 `onCartChanged` 函数。这种方式可以在组件的生命周期中存储状态更长久，从而使状态持久化。理想情况下，组件的传给 [runApp](https://api.flutter.dev/flutter/widgets/runApp.html) 的状态可以持久到整个应用的生命周期。
...
当父组件收到了 `onCartChanged` 回调，这会触发父组件重建并使用新的 `inCart` 值来创建 `ShoppingListItem` 的新实例。尽管父组件在重建时会创建子组件的实例，但是这种代价是很小的，因为重新 [RenderObject](https://api.flutter.dev/flutter/rendering/RenderObject-class.html)
时，框架会比较新组件和原始组件的差异，并且只渲染那些有差异的部分。
...
示例中 `ShoppingList` 继承自 [StatefulWidget](https://api.flutter.dev/flutter/widgets/StatefulWidget-class.html)，就是说这个组件会存储状态的变化，当 `ShoppingList` 首次插入到渲染树结构中，框架会调用 [createState](https://api.flutter.dev/flutter/widgets/StatefulWidget-class.html#createState) 函数创建一个 `_ShoppingListState` 的新实例来与树中的该位置相关联。(注意到 [State](https://api.flutter.dev/flutter/widgets/State-class.html) 的子类通常以下划线开头来命名，表示它们的实现细节是私有的)。当该组件的父组件重新创建时，父组件首先创建一个ShoppingList的实例，但是框架会复用之前创建的 `_ShoppingListState`，而不会重新调用 [createState](https://api.flutter.dev/flutter/widgets/StatefulWidget-class.html#createState) 。
...
[widget](https://api.flutter.dev/flutter/widgets/State-class.html#widget)属性，当父组件重新创建一个新的 `ShoppingList`，`_ShoppingListState` 会使用新的 widget 值来创建。如果你想在 [widget](https://api.flutter.dev/flutter/widgets/State-class.html#widget)属性变化时接收到通知，你可以继承 [didUpdateWidget](https://api.flutter.dev/flutter/widgets/State-class.html#didUpdateWidget) 方法，该方法会将 oldWidget 作为参数传递，这样你就可以比较 `oldWidget` 和新 [widget](https://api.flutter.dev/flutter/widgets/State-class.html#widget)。
...
当处理 `onCartChanged` 回调时，`_ShoppingListState` 通过增加或删除`购物车`中的产品来改变内部状态。
在调用 [setState](https://api.flutter.dev/flutter/widgets/State-class.html#setState) 时，会通知框架改变它的内部状态。调用 
[setState](https://api.flutter.dev/flutter/widgets/State-class.html#setState) 标志着这个组件是脏的，
在应用下次刷新屏幕时，组件也需要重新创建。如果你在改变组件的内部状态时忘了调用 [setState](https://api.flutter.dev/flutter/widgets/State-class.html#setState)，框架不知道组件是脏组件，也就不会重新 [build](https://api.flutter.dev/flutter/widgets/StatelessWidget/build.html) 组件，用户界面就不会对状态的改变做更新。
...
在 StatefulWidget 调用 [createState](https://api.flutter.dev/flutter/widgets/StatefulWidget-class.html#createState)
...
例如配置动画或订阅平台服务。通过调用父类的 [super.initState](https://api.flutter.dev/flutter/widgets/State-class.html#initState)方法，实现 [initState](https://api.flutter.dev/flutter/widgets/State-class.html#initState) 开始进行初始化。
...
当不再需要状态对象，框架会对该对象调用 [dispose](https://api.flutter.dev/flutter/widgets/State-class.html#dispose)
 方法。你可以继承来清理状态。例如，你可以实现 [dispose](https://api.flutter.dev/flutter/widgets/State-class.html#dispose)
 来取消计时器或者取消订阅平台服务。通过调用父类的 [super.dispose](https://api.flutter.dev/flutter/widgets/State-class.html#dispose) 方法，实现 [dispose](https://api.flutter.dev/flutter/widgets/State-class.html#dispose)来清理状态。
...
你可以使用 Keys 来控制当一个组件重新创建时，框架哪些组件是和其他组件想匹配。默认情况下，框架会根据它的
[运行时类型](https://api.flutter.dev/flutter/widgets/Widget-class.html#runtimeType) 和出现的顺序来匹配在当前的和以前创建的组件。
使用 [Keys](https://api.flutter.dev/flutter/widgets/Widget-class.html#runtimeType) 时，
框架要求组件的 key 和 [runtimeType](https://api.flutter.dev/flutter/widgets/Widget-class.html#runtimeType) 保持一致。
...
Key 在构建多个相同类型的组件的时候非常有用。例如，`ShoppingList` 组件，就需要构建多个
 ...
 * Without keys, the first entry in the current build would always sync with the
   ...
   当不使用 Key 时，即使列表的第一项在语义上只是滑出屏幕的可见区域，它也始终会与其上一级同步构建。
...
你可以使用全局键来唯一标识子组件。 全局键在整个组件结构中必须是全局唯一的，而不像本地键只需要兄弟节点中是唯一的。
