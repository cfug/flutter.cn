...
当你准备好为 Android 准备 **发布** 版本的时候，比如[发布到 Google Play Store][play]，按照此页面上的步骤操作。
...
* `uses-permission`：如果你的应用程序代码不需要访问 Internet，移除 `android.permission.INTERNET` [权限][permissiontag]。标准模版里面包含这个标记，是为了开启 Flutter 工具和正在运行的 app 之间的通信。
...
* `defaultConfig`：
  ...
  * `minSdkVersion` & `targetSdkVersion`：指定支持的最小的 API 版本，以及该 app 被设计来运行在哪个 API 版本。可以参考[版本文档][versions]中的 API 版本部分。
...
1. 在 `<app dir>/android/app/src/main/res/` 目录下, 把你的图标文件放在以[配置限定符][]命名的文件夹中。
...
要把 app 发布到 Play store，你需要给 app 一个数字签名。根据以下的说明来给你的 app 签名：
...
如果你已经有一个密钥库，跳到下一步。如果没有，在命令行中运行以下的命令来创建一个：
`keytool -genkey -v -keystore ~/key.jks -keyalg RSA -keysize 2048 -validity 10000 -alias key`
...
创建一个名为 `<app dir>/android/key.properties` 的，包含密钥库引用的文件：
...
```
   buildTypes {
       release {
           // TODO：为发布版本添加你自己的签名配置
           // 现在用调试密钥来签名，这样 `flutter run --release` 就可以工作了
           signingConfig signingConfigs.debug
       }
   }
```
...
reverse engineering.
默认情况下，Flutter 不会做混淆或者缩小 Android host。如果你打算使用第三方的 Java 或者 Android 库，你可能希望减小 APK 的大小或者是保护代码不被逆向工程。
...
关于混淆 Dart 代码的信息，参考 [Flutter wiki]({{site.github}}/flutter/flutter/wiki) 上的 [Obfuscating Dart Code]({{site.github}}/flutter/flutter/wiki/Obfuscating-Dart-Code)。
...
这个部分讲述如何构建一个APK的发布版本。如果在前面的部分中你已经完成了签名步骤，发布版本的 APK 会被签名。
...
这个部分描述了如何构建一个发布的 app bundle。如果在前面的部分你已经完成了签名步骤，发布的bundle 会被签名。
...
1. 运行 `flutter build appbundle`。（运行 `flutter build` 默认构建一个发布版本)。
...
1. 为了生成 bundle 的不同变体，你可以输入
   <nobr>`flutter build appbundle --release --target-platform=android-arm`。</nobr>
   它生成一个用于 android-arm 平台的 bundle。

The release bundle for your app is created at
`<app dir>/build/app/outputs/bundle/release/app.aab`.
你的 app 的发布 bundle 被创建于
`<app dir>/build/app/outputs/bundle/release/app.aab`。
...
一个 app bundle 可以用多种方式进行测试。这个部分描述了测试 app bundle 的几种方式。
